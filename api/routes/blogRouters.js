...
router.post("/blogs", upload.single("image"), async (req, res) => {
  try {
    ...
    console.log("Request Body:", { title, content, categories, author, is_publish });
    ...
    const sql = `
      INSERT INTO blog (title, content, image_url, categories, author, is_publish, created_at, updated_at) 
      VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
    `;
...
router.get("/blogs/active", async (req, res) => {
  try {
    // console.log("request coming");
    const sql =
      "SELECT * FROM blog where is_publish = '1' ORDER BY created_at DESC";
    const [rows] = await db.execute(sql);
    // console.log(rows);
    res.json(rows);
  } catch (error) {
...
router.put("/blogs/:id/status", async (req, res) => {
  const { id } = req.params;
  const { is_publish } = req.body; // expected: "Published" or "Draft"

  console.log("Update payload:", { id, is_publish });
  try {
    // Update blog status
    const [result] = await db.execute(
      "UPDATE blog SET is_publish = ? WHERE id = ?",
      [is_publish, id]
    );
